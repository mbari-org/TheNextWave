...
from .swift import SBGData, SWIFTArray, SWIFTData
...
def reprocess_swift_array(
    ...
    for swift_num in range(22, 26):
        ...
        # Some sources (including certain MATLAB .mat exports) may have slightly
        # different-length arrays across channels. Trim to a common minimum length
        # so downstream vectorized operations stay aligned.
        lens = [
            int(np.size(t_us)),
            int(np.size(z)),
            int(np.size(u)),
        ...
        n_common = int(np.min(lens)) if lens else 0
        if n_common <= 0:
            print(f'{swift_name}: empty data after alignment')
            continue
        if any(L != n_common for L in lens):
            t_us = t_us[:n_common]
            z = z[:n_common]
            u = u[:n_common]
            v = v[:n_common]
        ...
        try:
            ...
            # Direction convention: sbg_waves() mirrors SBGWaves.m with recip=true,
            # so Dp is already nautical degrees True, direction waves are coming FROM.
